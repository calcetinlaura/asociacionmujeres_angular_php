...
switch ($method) {
    case 'GET':
      if (isset($_GET['latest'])) {
        ...
        if ($latestYear) {
            ...
            $stmt->prepare("SELECT * FROM movies WHERE year = ?");
    ...
    case 'POST':
        ...
        $stmt = $connection->prepare("INSERT INTO movies (title, author, gender, year) VALUES (?, ?, ?, ?)");
    ...
    case 'PATCH':
        $data = json_decode(file_get_contents('php://input'), true);
        $id = $_GET['id'];
        $stmt = $connection->prepare("UPDATE movies SET title = ?, author = ?, gender = ?, year = ? WHERE id = ?");
